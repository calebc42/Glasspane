...
The Glasspane Emacs client — a Tier-1 app built on the **EABP core** (vendored
here as the `eabp` git submodule). Pure elisp: it `(require 's the core and
drives the Android companion over the EABP wire. **All Kotlin lives in the
[eabp] repo**; this repo is elisp only.

## Layout

- `emacs/apps/glasspane/` — the org app (reader, rich org, tables, journal,
  saved views, SRS, notes/backlinks, clock, demo tour…)
- `emacs/apps/eabp-*.el` — reference Tier-1 apps bundled with Glasspane
...
- `emacs/build-bundle.el` — regenerates `glasspane.el` (app-only; the bundle
  opens with `(require 'eabp-core)`)
- `eabp/` — the EABP core (foundation + companion), a git submodule
...
```elisp
(require 'eabp-core)   ; from eabp-core.el (the eabp repo / submodule)
